...
resource "proxmox_virtual_environment_file" "cloud_image" {
  ...
  source_file {
    path = "${var.cloud_image}"
    file_name = "freeipa-server-cloudimg.img"
...
resource "proxmox_virtual_environment_vm" "freeipa-masters" {
  count = var.master_count
  name        = "freeipa-server-0${count.index+1}"
  description = "Free IPA server - managed by Terraform"
  tags        = ["terraform", "freeipa"]
